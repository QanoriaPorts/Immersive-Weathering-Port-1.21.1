...
public abstract class BuiltinBlockGrowth implements IBlockGrowth {
    ...
    // 1.21 partialDispatch lookup must return DataResult<MapCodec<? extends E>>
    // (was DataResult<Codec<? extends E>> in 1.20.x), so the inner record
    // builder is now created via mapCodec() instead of create().
    public static final Codec<BuiltinBlockGrowth> CODEC =
            Codec.STRING.partialDispatch("builtin", b -> DataResult.success(b.getName()),
                    n -> {
                        var factory = BuiltinGrowthsRegistry.BUILTIN_GROWTHS.get(n);
                        if (factory == null) {
                            return DataResult.error(() -> "No builtin growth found with id " + n);
                        }
                        com.mojang.serialization.MapCodec<BuiltinBlockGrowth> codec = RecordCodecBuilder.mapCodec(i -> i.group(
