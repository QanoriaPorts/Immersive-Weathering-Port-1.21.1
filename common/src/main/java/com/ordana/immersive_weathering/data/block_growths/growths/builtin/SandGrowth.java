...
public class SandGrowth extends BuiltinBlockGrowth {
    ...
    public @Nullable Iterable<Block> getOwners() {
        // 1.21 changed when tags become bound relative to the JSON reload phase
        // we run from. The base mod calls .get() directly; in 1.21 the Optional
        // can still be empty here, so we ifPresent() through both tags and
        // tolerate either being unbound.
        List<Block> blocks = new ArrayList<>();
        BuiltInRegistries.BLOCK.getTag(ModTags.SANDABLE).ifPresent(set ->
                set.stream().forEach(h -> blocks.add(h.value())));
        BuiltInRegistries.BLOCK.getTag(ModTags.SANDY).ifPresent(set ->
                set.stream().forEach(h -> blocks.add(h.value())));
        return blocks;
    }
