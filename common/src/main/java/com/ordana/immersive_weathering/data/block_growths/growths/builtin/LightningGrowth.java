...
public class LightningGrowth extends BuiltinBlockGrowth {
    ...
    public @Nullable Iterable<Block> getOwners() {
        // Same defensive lookup as SandGrowth - tags may be unbound at this
        // point in the 1.21 reload sequence.
        List<Block> blocks = new ArrayList<>();
        BuiltInRegistries.BLOCK.getTag(BlockTags.SAND).ifPresent(set ->
                set.stream().forEach(h -> blocks.add(h.value())));
        BuiltInRegistries.BLOCK.getTag(ModTags.CRACKABLE).ifPresent(set ->
                set.stream().forEach(h -> blocks.add(h.value())));
        return blocks;
    }
